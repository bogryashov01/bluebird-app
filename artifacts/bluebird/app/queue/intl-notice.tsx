...
export default function InternationalNoticeScreen() {
  ...
  const occupantLimitExceeded = bringingPet && passengers >= MAX_OCCUPANTS;
  ...
  const handleContinueBase = () => {
    if (!flightId || occupantLimitExceeded) return;
    router.push({
      ...
      <View style={[styles.footer, { paddingBottom: bottomPad + 12 }]}>
        {occupantLimitExceeded && (
          <Text style={[styles.hint, { color: colors.coral }]}>
            A pet counts as one occupant, so bookings with a pet may include at most {MAX_OCCUPANTS - 1} passengers.
          </Text>
        )}
        ...
        <SecondaryButton
          ...
          disabled={occupantLimitExceeded}
...
const styles = StyleSheet.create({
  ...
  hint: { fontFamily: 'Inter_500Medium', fontSize: 12.5, lineHeight: 18, textAlign: 'center' },
});
