...
if (bareCount === 0) {
  console.log("patch-generated: already up to date, nothing to do.");
  process.exit(0);
}

// Replace every bare occurrence.  There are no already-patched occurrences
// (REPLACEMENT contains NEEDLE as a substring only if Partial< were absent,
// which it isn't — REPLACEMENT is "query?:Partial<UseQueryOptions<" while
// NEEDLE is "query?:UseQueryOptions<", a strictly distinct string).
const afterReplace = original.replaceAll(NEEDLE, REPLACEMENT);

// Add the matching closing angle bracket on every affected line
...
console.log(
  `patch-generated: wrapped ${bareCount} UseQueryOptions occurrence(s) in Partial<>.`
);
